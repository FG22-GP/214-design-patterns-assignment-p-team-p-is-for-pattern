...
void TextureManager::CleanUpFunction() {
    for (auto id : textureMap) {
        auto* textureToDestroy = static_cast<SDL_Texture*>(id.second);
...
bool TextureManager::LoadText(std::string fileName, std::string id, Color textColor, int fontSize, std::string text,
                              Vector2D& dimensions) {
...
void TextureManager::Draw(std::string id, Vector2D imagePosition, Vector2D imageSize, FlipState flip) {
    ...
    targetRectangle.x = imagePosition.GetX(); // these are just rounded 
    targetRectangle.y = imagePosition.GetY();
    targetRectangle.w = imageSize.GetX();
    targetRectangle.h = imageSize.GetY();
    auto* texture = static_cast<SDL_Texture*>(textureMap[id]);
    if (texture != nullptr) {
...
Uint8 TextureManager::SetAlpha(std::string id, Uint8 value) {
    auto* texture = static_cast<SDL_Texture*>(textureMap[id]);
    Uint8 currentAlpha = 0;
    SDL_GetTextureAlphaMod(texture, &currentAlpha);
    if (texture && currentAlpha > 0) {
        if (currentAlpha -= value < 0) SDL_SetTextureAlphaMod(texture, 0);
        else {
            const auto val = currentAlpha -= value;
            SDL_SetTextureAlphaMod(texture, val);
            
        }
    }
    
    printf("Current Alpha: %d", currentAlpha);
...
void TextureManager::DrawFrame(std::string id, Vector2D imagePosition, Vector2D imageSize, int currentRow,
                               int currentFrame, FlipState flip) {
    SDL_Rect targetRectangle;
...
void TextureManager::ClearFromTextureMap(std::string textureID) {
    auto* texture = static_cast<SDL_Texture*>(textureMap[textureID]);

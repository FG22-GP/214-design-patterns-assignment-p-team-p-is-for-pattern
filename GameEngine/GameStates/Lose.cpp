...
Lose::Lose(GameManager* manager) : GameState(manager) {
    ...
    TheTextureManager::Instance()->LoadText("font/Silkscreen-Regular.ttf", "restart", lostTextColor, 30, "Press Space to Play New Level", restartTextSize);
    TheTextureManager::Instance()->LoadText("font/Silkscreen-Regular.ttf", "retry", lostTextColor, 30, "Press Enter to Retry Current Level", restartTextSize);
}
...
void Lose::Update() {
    ...
    TheTextureManager::Instance()->Draw("restart", Vector2D(WindowSizeX / 4, WindowSizeY / 4 + restartTextSize.GetY()), restartTextSize);
